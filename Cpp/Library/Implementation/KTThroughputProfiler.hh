...
namespace Nymph
{
    /*!
     ...
     Slots:
     - "start": void (EggHeader*) -- Start the timer
     - "data": void (DataHandle) -- Increment the counter on the number of data slices
     - "stop": void () -- Stop the timer

    */
    class ThroughputProfiler : public Processor
    {

        public:
            ThroughputProfiler(const std::string& name = "throughput-profiler");
            virtual ~ThroughputProfiler();
            ...
            void StartProfiling(DataHandle data);

            void Data(DataHandle data);
    ...
    inline bool ThroughputProfiler::GetOutputFileFlag() const
    {
        return fOutputFileFlag;
    }

    inline void ThroughputProfiler::SetOutputFileFlag(bool flag)
    {
        fOutputFileFlag = flag;
        return;
    }

    inline const std::string& ThroughputProfiler::GetOutputFilename() const
    {
        return fOutputFilename;
    }

    inline void ThroughputProfiler::SetOutputFilename(const std::string& fname)
    {
